...
try:
    ...
    log("FATAL: ADK libraries import failed. AI agent functionality will be disabled.", is_error=True)
...
async def _validate_agent_prerequisites(remediation_id: str, runner, session, agent_type: str) -> tuple:
    """Validates that all prerequisites for agent execution are met."""
...
async def _create_agent_event_stream(runner, session_id: str, user_id: str, user_query: str, remediation_id: str):
    """Creates the async event stream for agent execution."""
...
async def _check_event_limit(event_count: int, agent_type: str, max_events_limit: int) -> tuple:
    ...
    if event_count > max_events_limit:
        ...
        final_response = (f"\n\n⚠️ Note: Agent execution was terminated early after reaching the maximum limit "
                          f"of {max_events_limit} events. The solution may be incomplete.")
...
def _process_function_calls(event, agent_type: str, agent_tool_calls_telemetry: list):
...
def _process_function_responses(event, agent_type: str, agent_tool_calls_telemetry: list):
...
async def _process_agent_event(event, event_count: int, agent_type: str, max_events_limit: int,
                               agent_tool_calls_telemetry: list) -> tuple:
    """Processes a single agent event and returns updated state."""
...
async def _handle_agent_exception(e: Exception, events_async, remediation_id: str) -> bool:
    """Handles exceptions during agent execution. Returns True if execution should continue."""
    error_message = str(e)
...
async def process_agent_run(runner, session, user_query, remediation_id: str, agent_type: str = None) -> str:
    ...
    session_id, user_id = await _validate_agent_prerequisites(remediation_id, runner, session, agent_type)
    ...
    events_async = await _create_agent_event_stream(runner, session_id, user_id, user_query, remediation_id)
    ...
    try:
        ...
        should_continue = await _handle_agent_exception(e, events_async, remediation_id)
...
def _extract_analytics_data(agent_summary_str: str):
...
def _extract_pr_body(agent_summary_str: str) -> str:
...
def run_ai_fix_agent(repo_root: Path, fix_system_prompt: str, fix_user_prompt: str, remediation_id: str) -> str:
    ...
    try:
        ...
        return _extract_pr_body(agent_summary_str)

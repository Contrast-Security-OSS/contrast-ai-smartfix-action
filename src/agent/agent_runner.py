...
class AgentRunner:
    ...
    async def _create_agent(self, target_folder: Path, remediation_id: str, agent_model: str, agent_type: str = "fix", system_prompt: Optional[str] = None) -> Optional[Agent]:
        """Creates an ADK Agent (either 'fix' or 'qa')."""
        mcp_tools = await self._get_mcp_tools(target_folder, remediation_id)
        if not mcp_tools:
        ...
        try:
            model_instance = LiteLlm(model=agent_model, stream_options={"include_usage": True})
            root_agent = Agent(
            ...
            debug_log(f"Created {agent_type} agent ({agent_name}) with model {agent_model}")
            return root_agent
        except Exception as e:
    ...
    async def _process_agent_run(self, runner: Runner, session, user_query, remediation_id: str, agent_type: str, max_events_per_agent: int) -> str:
        ...
        try:
            async for event in events_async:
                ...
                if event_count > max_events_per_agent:
                    log(f"\n⚠️ Reached maximum event limit of {max_events_per_agent} for {agent_type.upper()} agent. Stopping agent execution early.")
                    final_response += f"\n\n⚠️ Note: Agent execution was terminated early after reaching the maximum limit of {max_events_per_agent} events. The solution may be incomplete."
    ...
    async def _run_agent_internal_with_prompts(self, agent_model: str, agent_type: str, repo_root: Path, query: str, system_prompt: str, remediation_id: str, max_events_per_agent: int) -> str:
        ...
        agent = await self._create_agent(repo_root, remediation_id, agent_model, agent_type=agent_type, system_prompt=system_prompt)
        if not agent:
        ...
        summary = await self._process_agent_run(runner, session, query, remediation_id, agent_type, max_events_per_agent)
        
        return summary

    def _run_agent_in_event_loop(self, *args, **kwargs):
        """
        Wrapper function to run _run_agent_internal_with_prompts in a controlled event loop.
        Handles proper setup and cleanup of the event loop and tasks.
        
        Args:
            *args, **kwargs: Arguments to pass to _run_agent_internal_with_prompts
            
        Returns:
            The result returned by _run_agent_internal_with_prompts
        """
        result = None
        ...
        try:
            ...
            task = loop.create_task(self._run_agent_internal_with_prompts(*args, **kwargs))
...
#        processed_user_prompt = process_fix_user_prompt(fix_user_prompt) # TODO: call this when making the fix agent
        ...
        try:
            ...
            agent_summary_str = self._run_agent_in_event_loop(
                agent_model,
                'fix', 
                repo_root, 
                fix_agent.user_prompt, 
                fix_agent.system_prompt, 
                remediation_id,
    ...
    def run_qa_agent(self, build_output: str, changed_files: List[str], build_command: str, repo_root: Path, remediation_id: str, agent_model: str,qa_history: Optional[List[str]] = None, qa_system_prompt: Optional[str] = None, qa_user_prompt: Optional[str] = None) -> str:
        """
        ...
        if qa_system_prompt and qa_user_prompt:
            ...
            qa_query = self._process_qa_user_prompt(qa_user_prompt, changed_files, build_output, qa_history_section)
        else:
